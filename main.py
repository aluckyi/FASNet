...
def load_dataset(dataset):
    ...
    transform = transforms.Compose([
        ext_transforms.Resize((args.height, args.width), Image.BILINEAR, Image.NEAREST),
        ext_transforms.ToTensors()
    ])

    train_transform = transforms.Compose([
        # ext_transforms.RandomCrop((args.crop_height, args.crop_width)),
        ext_transforms.Resize((args.height, args.width), Image.BILINEAR, Image.NEAREST),
        ext_transforms.RandomColorJitter(*args.color_jitter),
        ext_transforms.RandomHorizontalFlip(args.hflip_prob),
        ext_transforms.ToTensors()
    ])
    ...
    # Load the train set as tensors for the weighing technique
    weighing_set = dataset(
        args.dataset_dir,
        transform=transform)
    weighing_loader = data.DataLoader(
        weighing_set,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=args.workers)
    ...
    if args.weighing.lower() == 'enet':
        class_weights = enet_weighing(weighing_loader, num_classes)
    elif args.weighing.lower() == 'mfb':
        class_weights = median_freq_balancing(weighing_loader, num_classes)
    else:
        class_weights = None
...
def train(train_loader, val_loader, class_weights, class_encoding):
    ...
    # FSSNet authors used Adam as the optimizer
